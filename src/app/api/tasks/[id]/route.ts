...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const task = await db.task.update({ where: { id }, data: { title: body.title !== undefined ? body.title.trim() : undefined, description: body.description !== undefined ? body.description.trim() : undefined, status: body.status, dueDate: body.dueDate ? new Date(body.dueDate) : undefined, reminderAt: body.reminderAt !== undefined ? (body.reminderAt ? new Date(body.reminderAt) : null) : undefined, taskTypeId: body.taskTypeId !== undefined ? Number(body.taskTypeId) : undefined, assigneeEmails: body.assigneeEmails !== undefined ? emails.join(', ') : undefined, userGroupId: null, completedByEmail: body.status !== undefined ? (body.status === 'COMPLETED' ? existing.completedByEmail : null) : undefined, ...(reminderChanged ? { reminderSentAt: null } : {}), ...(dueChanged ? { dueEmailSentAt: null } : {}) }, include: { taskType: true } });
