...
export default function DmsPage() {
  ...
  const tiles: { label: string; value: number; Icon: LucideIcon }[] = [
    { label: 'Total users', value: data.users.length, Icon: UsersRound },
    { label: 'Active users', value: activeUsers, Icon: UserRoundCheck },
    { label: 'In active users', value: inactiveUsers, Icon: CheckCircle2 },
    { label: 'Indexed documents', value: totalDocuments, Icon: Database },
  ];
  ...
  return <div className="min-w-0 space-y-6 p-4 sm:p-6 lg:p-8">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[.22em] text-indigo-500">App Progress Dashboard</p><h1 className="mt-2 text-3xl font-bold text-[var(--text-title)]">Document Management System</h1><p className="mt-2 max-w-2xl text-sm text-[var(--text-muted)]">Monitor DMS users and monthly indexing activity from the two source reports.</p></div>
      <div className="flex flex-col gap-2 sm:flex-row"><AppDashboardSelector /><input ref={inputRef} hidden type="file" multiple accept=".xlsx" onChange={upload} /><button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} className="dashboard-primary-button flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold disabled:opacity-60">{uploading ? <Loader2 size={17} className="animate-spin" /> : <Upload size={17} />}Upload both reports</button></div>
    </header>
    ...
    <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {tiles.map(({ label, value, Icon }) => <div key={label} className="dashboard-card rounded-2xl p-5"><div className="flex items-center justify-between text-sm text-[var(--text-muted)]"><span>{label}</span><Icon size={19} className="text-indigo-500" /></div><strong className="mt-4 block text-3xl text-[var(--text-title)]">{value.toLocaleString()}</strong></div>)}
    </section>
    {loading ? <div className="flex h-64 items-center justify-center"><Loader2 className="animate-spin text-indigo-500" /></div> : <section className="grid gap-5 xl:grid-cols-3">
      <article className="dashboard-card rounded-2xl p-5 xl:col-span-2"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-[var(--text-title)]">Monthly index count</h2><p className="text-xs text-[var(--text-muted)]">Total indexed documents by selected month</p></div><MonthFilter months={months} selected={selectedMonths} onChange={setSelectedMonths} /></div><div className="h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 24, right: 10, left: 0, bottom: 0 }}><CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" /><XAxis dataKey="month" tick={{ fill: 'var(--chart-text)' }} /><YAxis tick={{ fill: 'var(--chart-text)' }} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="documents" fill="#6366f1" radius={[6,6,0,0]}><LabelList dataKey="documents" position="top" fill="var(--chart-value)" fontSize={11} formatter={(value: unknown) => Number(value).toLocaleString()} /></Bar></BarChart></ResponsiveContainer></div></article>
      <article className="dashboard-card rounded-2xl p-5"><h2 className="font-semibold text-[var(--text-title)]">User status</h2><p className="text-xs text-[var(--text-muted)]">Active versus in active users</p><div className="h-80"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieData} dataKey="value" nameKey="name" innerRadius={65} outerRadius={105} label={({ name, value }) => `${name}: ${value}`} labelLine={false}>{pieData.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip contentStyle={tooltipStyle} /></PieChart></ResponsiveContainer></div></article>
      <article className="dashboard-card rounded-2xl p-5 xl:col-span-3"><h2 className="font-semibold text-[var(--text-title)]">Top indexing users</h2><div className="mt-4 h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={topUsers} layout="vertical" margin={{ left: 20, right: 55 }}><CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" /><XAxis type="number" tick={{ fill: 'var(--chart-text)' }} /><YAxis type="category" dataKey="name" width={120} tick={{ fill: 'var(--chart-text)', fontSize: 11 }} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="documents" fill="#0ea5e9" radius={[0,6,6,0]}><LabelList dataKey="documents" position="right" fill="var(--chart-value)" fontSize={11} formatter={(value: unknown) => Number(value).toLocaleString()} /></Bar></BarChart></ResponsiveContainer></div></article>
    </section>}
    <section className="dashboard-card overflow-hidden rounded-2xl"><div className="flex flex-col gap-3 border-b border-white/10 p-5 md:flex-row md:items-center md:justify-between"><div><h2 className="font-semibold text-[var(--text-title)]">DMS user details</h2><p className="text-xs text-[var(--text-muted)]">Showing {visibleUsers.length.toLocaleString()} of {data.users.length.toLocaleString()} users</p></div><div className="flex flex-col gap-2 sm:flex-row"><label className="dashboard-control flex items-center gap-2 rounded-xl px-3"><Search size={16} /><input className="min-w-0 bg-transparent py-2.5 text-sm outline-none" placeholder="Search users..." value={search} onChange={(event) => setSearch(event.target.value)} /></label><select className="dashboard-control rounded-xl px-3 py-2.5 text-sm" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">All statuses</option><option>Active</option><option>In Active</option></select></div></div>
