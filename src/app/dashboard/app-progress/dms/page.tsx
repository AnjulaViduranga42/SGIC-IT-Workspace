...
export default function DmsPage() {
  ...
  return <div className="min-w-0 space-y-7 transition-opacity duration-300 ease-out">
    ...
    {loading ? <div className="flex h-64 items-center justify-center"><Loader2 className="animate-spin text-indigo-500" /></div> : <section className="grid gap-5 xl:grid-cols-3">
      <article className="glass-panel rounded-2xl border border-white/10 p-5 xl:col-span-2"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-white">Monthly index count</h2><p className="mt-1 text-xs text-slate-400">Click a bar to view underlying records</p></div><MonthFilter months={months} selected={selectedMonths} onChange={setSelectedMonths} /></div><div className="h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} margin={{ top: 24, right: 10, left: 0, bottom: 0 }}><CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" /><XAxis dataKey="month" tick={{ fill: 'var(--chart-text)', fontSize: 11 }} /><YAxis tick={{ fill: 'var(--chart-text)', fontSize: 11 }} /><Tooltip contentStyle={tooltipStyle} /><Bar className="cursor-pointer" dataKey="documents" fill="#6366f1" radius={[4,4,0,0]} onClick={(entry) => openMonth(String((entry as { payload?: { monthKey?: string } }).payload?.monthKey || ''))}><LabelList dataKey="documents" position="top" fill="var(--chart-value)" fontSize={10} formatter={(value: unknown) => Number(value).toLocaleString()} /></Bar></BarChart></ResponsiveContainer></div></article>
      <article className="glass-panel rounded-2xl border border-white/10 p-5"><h2 className="font-semibold text-white">User status</h2><p className="mt-1 text-xs text-slate-400">Click a segment to view users</p><div className="h-80"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie className="cursor-pointer" data={pieData} dataKey="value" nameKey="name" innerRadius={62} outerRadius={96} paddingAngle={3} label={({ name, value }) => `${name}: ${value}`} labelLine={false} onClick={(entry) => openStatus(String((entry as { name?: string }).name || ''))}>{pieData.map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip contentStyle={tooltipStyle} /></PieChart></ResponsiveContainer></div></article>
      <article className="glass-panel rounded-2xl border border-white/10 p-5 xl:col-span-3"><h2 className="font-semibold text-white">Top indexing users</h2><p className="mt-1 text-xs text-slate-400">Click a bar to view the user’s index records</p><div className="mt-5 h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={topUsers} layout="vertical" margin={{ left: 20, right: 55 }}><CartesianGrid stroke="var(--chart-grid)" strokeDasharray="3 3" /><XAxis type="number" tick={{ fill: 'var(--chart-text)', fontSize: 11 }} /><YAxis type="category" dataKey="name" width={120} tick={{ fill: 'var(--chart-text)', fontSize: 11 }} /><Tooltip contentStyle={tooltipStyle} /><Bar className="cursor-pointer" dataKey="documents" fill="#6366f1" radius={[0,4,4,0]} onClick={(entry) => openIndexer(String((entry as { payload?: { name?: string } }).payload?.name || ''))}><LabelList dataKey="documents" position="right" fill="var(--chart-value)" fontSize={11} formatter={(value: unknown) => Number(value).toLocaleString()} /></Bar></BarChart></ResponsiveContainer></div></article>
    </section>}
